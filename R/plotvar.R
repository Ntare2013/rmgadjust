...
#' @export
#' @param 'which' is a charater string and you can check rmgarch package
#' @param '...' is a charater string and you can check rmgarch package
#' @param 'series' is a charater string and you can check rmgarch package
...
.plot.dccfit.5 = function(x, series, ...)
{
  ...
  q05 = rugarch::qdist(distribution = dm, p = 0.05, mu = dport[,1], sigma = dport[,2], lambda = -0.5,
                       skew = dport[,3], shape = dport[,4])
  plot(zoo(port, xDates), col = "gray74", ylab = "", xlab = "", main = "Portfolio with 1% and 5% VaR limits",
       cex.main = 0.8)
  lines(zoo(q01, xDates), col = "blue")
  lines(zoo(q05, xDates), col = "red")
  legend("topleft", legend = c("1% VaR", "5% VaR", "Returns"), lty = c(1,1,1), bty = "n", col = c("blue", "red", "gray74"), cex = 0.9)
  title(xlab = "Years", ylab = "Portfolio returns & VaR")
